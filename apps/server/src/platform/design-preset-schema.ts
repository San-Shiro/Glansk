 ...
 * Enforces cohesive, curated visual tokens per widget or package,
 * completely eliminating uncoordinated ad-hoc colors and the generic "cheap AI look".
 */
...
export interface WidgetDesignPreset {
  ...
  readonly typography?: {
    readonly fontFamily?: "sans" | "mono" | "display";
    readonly fontScale?: "compact" | "standard" | "prominent";
  };
  ...
  /** Widget-specific custom token overrides (e.g. gauge arc color, glow intensity) */
  readonly widgetTokens?: Readonly<Record<string, string | number | boolean>>;
}

/**
 * Built-in Curated Design Presets.
 * Guaranteed to have high contrast, harmonious palettes, and pixel-precise readability.
 */
export const GULF_RACING_PRESET: WidgetDesignPreset = {
  presetVersion: 1,
  id: "gulf-racing",
  name: "Gulf GT Racing",
...
export const BUILTIN_DESIGN_PRESETS: Readonly<Record<string, WidgetDesignPreset>> = {
  "gulf-racing": GULF_RACING_PRESET,
...
/**
 * Validates whether an object conforms to the WidgetDesignPreset structure.
 */
export function validateDesignPreset(preset: unknown): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
  ...
  if (!p.palette || typeof p.palette !== "object") {
    ...
    const pal = p.palette as Record<string, any>;
...
/**
 * Resolves CSS custom properties from an active preset and optional color overrides.
 */
export function resolvePresetCssVariables(
  preset?: WidgetDesignPreset | null,
  colorOverrides?: Record<string, string>
): Record<string, string> {
  const active = preset ?? GULF_RACING_PRESET;
  ...
  // Apply sparse user overrides on top
  if (colorOverrides) {
...
/**
 * Applies a curated design preset to a widget instance configuration.
 */
export function applyPresetToWidget(
  widget: { config?: Record<string, any> },
  presetId: string,
